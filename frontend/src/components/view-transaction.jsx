...
const ViewTransaction = ({ data, isOpen, setIsOpen }) => {
  ...
  const longDateString = new Date(data?.createdat).toLocaleDateString("en-US",{dataStyle:"full"})

  const longTimeString = new Date(data?.createdat).toLocaleTimeString("en-US");
  return (
    <DialogWrapper isOpen={isOpen} closeModal={closeModal}>
        <DialogPanel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white dark:bg-slate-900 p-6 text-left align-middle shadow-xl">
          <DialogTitle
            as="h3"
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-2 text-gray-600 dark:text-gray-500 border-y border-gray-300 dark:border-gray-700 py-3">
              <p>{data?.source}</p>
