        ...
        {data?.map((item, index)=> (
          <div key={index + item?.account_name} className="flex items-center justify-between mt-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-black dark:text-gray-400 text-lg">{item.account_name}</p>
                <span className=" text-gray-600">{maskAccountNumber(item.account_number)}</span>
              </div>
            </div>
            <div>
              <p className="text-xl text-black dark:text-gray-400 font-medium">₹{formatCurrency(item?.account_balance)}</p>
              <span className="text-xl text-gray-600 dark:text-violet-700">Account Balance</span>
            </div>
          </div>
        ))}
      </div>
...
//     icon: (
//       <div
//         className="w-12 h-12 bg-rose-600 text-white flex items-center justify-center rounded-full"
//       >
